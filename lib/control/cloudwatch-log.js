...
class CloudWatchLog {
  ...
  async put(logEvents, streamInfo = {}) {
    ...
    if (streamInfo.streamName) this.nextSequenceToken = streamInfo.sequenceToken;
    if (!this.logStreamName) await this.start();

    const cloudWatchLogsSdk = this.cloudWatchLogsSdk;
    const logGroupName = this.logGroupName;
    const logStreamName = streamInfo.streamName || this.logStreamName;
    const sequenceToken = streamInfo.sequenceToken || this.nextSequenceToken;
    const opts = {
      cloudWatchLogsSdk,
      logGroupName,
      logStreamName,
